...
#'
#' @param width Numeric value specifying the width of the new plotting device in
#' centimeters.
...
#'
#' @return A ggplot2 theme object with customised settings for plot title, axis
#' titles, axis text, legend text, legend title, major grid lines, legend key
...
#'   \item `axis.title.x`: Sets the font size of the x-axis title (default is
#'   10).
#'   \item `axis.title.y`: Sets the font size of the y-axis title (default is
#'   10).
...
#'   \item `legend.key`: Sets the legend key background (default is
#'   "transparent").
#'   \item `legend.position`: Positions the legend (default coordinates are
#'   (0.8, 0.9)).
#' }
#'
#' @examples
